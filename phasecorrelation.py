...
import time
import cv2
from ImageFusion import *

class phaseCorrelation(ImageFusion):

    def __init__(self):
        jarpath = os.path.join(os.path.abspath('.'), './jar/')  # jar所在路径
        startJVM(getDefaultJVMPath(), "-ea", "-Djava.class.path=%s" % (jarpath + 'Stitching1_.jar'))  # 包名
    ...
    def flowStitch(self, fileAddress):
        ...
        for element in elements:
            offsetList.append([int(element.getModel().tx), int(element.getModel().ty)])

        offsets.append(offsetList[0])
        for j in range(1, len(offsetList)):
            offsets.append([offsetList[j][0] - offsetList[j - 1][0], offsetList[j][1] - offsetList[j - 1][1]])

        endTime = time.time()
        self.printAndWrite("The time of registering is " + str(endTime - startTime) + "s")
        self.printAndWrite("  The offsetList is " + str(globaloffsets))
